...
export default function AsistenciaPage() {
  ...
  return (
    <div style={{
      ...
      {loading ? (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh', color: '#fdf5e6', fontFamily: 'inherit' }}>
        ...
        <>
          ...
          <div style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
            <div>
              <p style={{ fontSize: '0.9rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: '0.5rem' }}>Operatividad</p>
              ...
              <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: '0.5rem', fontSize: '1.35rem' }}>Control horario de entrada y salida del personal.</p>
            </div>
            ...
            {userRole === 'admin' && (
              <div style={{ background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)', padding: '0.5rem 1rem', borderRadius: '4px', flexShrink: 0 }}>
                <span style={{ color: '#a78bfa', fontSize: '0.98rem', fontWeight: 700, letterSpacing: '0.1em' }}>● MODO ADMINISTRADOR</span>
              </div>
            )}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: userRole === 'admin' ? '1fr' : 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2.5rem' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    <h3 style={{ fontSize: '1.35rem', fontWeight: 700, color: '#fdf5e6', margin: 0 }}>SEGUIMIENTO DE PERSONAL</h3>
                    {filterEmployee === 'all' ? (
                      <p style={{ margin: '0.15rem 0 0', fontSize: '0.9rem', color: '#60a5fa', letterSpacing: '0.05em' }}>
                      ...
                      <p style={{ margin: '0.15rem 0 0', fontSize: '0.9rem', color: '#fbbf24', letterSpacing: '0.05em' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <select 
                    ...
                    style={{ background: '#0a0a0f', border: '1px solid rgba(255,255,255,0.1)', color: '#fdf5e6', padding: '0.4rem 0.8rem', fontSize: '1.13rem', outline: 'none', cursor: 'pointer' }}
                  >
              ...
              <div style={{ background: 'rgba(255,255,255,0.01)', padding: '0.75rem 1.5rem', display: 'flex', gap: '2rem', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ fontSize: '0.83rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total Horas</span>
                  <span style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fbbf24' }}>{stats.total.toFixed(1)}h</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ fontSize: '0.83rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Promedio por Turno</span>
                  <span style={{ fontSize: '1.5rem', fontWeight: 700, color: '#a78bfa' }}>{stats.promedio.toFixed(1)}h</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ fontSize: '0.83rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Registros</span>
                  <span style={{ fontSize: '1.5rem', fontWeight: 700, color: '#60a5fa' }}>{adminHistory.length}</span>
                </div>
              </div>
                  ...
                  <tbody>
                    {adminHistory.map((record, i) => (
                      <tr key={record.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.005)' }}>
                        <td style={{ padding: '0.9rem 1.5rem', color: '#a78bfa', fontSize: '1.2rem', fontWeight: 600 }}>{record.nombre_completo}</td>
                        <td style={{ padding: '0.9rem 1.5rem', color: 'rgba(255,255,255,0.6)', fontSize: '1.13rem' }}>{formatDate(record.hora_entrada)}</td>
                        <td style={{ padding: '0.9rem 1.5rem', color: '#fdf5e6', fontSize: '1.2rem' }}>{formatTime(record.hora_entrada)}</td>
                        <td style={{ padding: '0.9rem 1.5rem', color: 'rgba(255,255,255,0.3)', fontSize: '1.2rem' }}>{record.hora_salida ? formatTime(record.hora_salida) : '--:--'}</td>
                        <td style={{ padding: '0.9rem 1.5rem', textAlign: 'right', color: '#fbbf24', fontSize: '1.27rem', fontWeight: 700 }}>{record.total_horas || 0}h</td>
                        <td style={{ padding: '0.9rem 0.75rem', textAlign: 'right' }}>
                          <div style={{ display: 'inline-flex', gap: '0.5rem' }}>
                            {isSuperAdmin && (
                              <button 
                                onClick={() => setEditingRecord({...record})}
                                style={{ background: 'transparent', border: '1px solid rgba(251,191,36,0.25)', color: 'rgba(251,191,36,0.6)', padding: '0.25rem 0.6rem', fontSize: '0.9rem', cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.06em', transition: 'all 0.15s' }}
                            ...
                            <button
                              ...
                              style={{ background: 'transparent', border: '1px solid rgba(248,113,113,0.25)', color: 'rgba(248,113,113,0.6)', padding: '0.25rem 0.6rem', fontSize: '0.9rem', cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.06em', transition: 'all 0.15s' }}
                    ...
                    {adminHistory.length === 0 && (
                      <tr>
                        <td colSpan={5} style={{ padding: '2.5rem', textAlign: 'center', color: 'rgba(255,255,255,0.3)', fontSize: '1.2rem' }}>No se encontraron registros activos.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <div style={{ background: 'rgba(96,165,250,0.05)', border: '1px solid rgba(96,165,250,0.2)', padding: '1.5rem' }}>
            ...
            <h4 style={{ color: '#fdf5e6', fontSize: '1.35rem', margin: '0 0 0.5rem' }}>Estado del Sistema</h4>
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '1.13rem', lineHeight: 1.6 }}>El registro de asistencia se valida mediante el servidor de Supabase. Asegúrate de tener una conexión estable al marcar tus tiempos.</p>
          </div>
          
          <div style={{ padding: '0 1rem' }}>
            <h4 style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '1.5rem' }}>Normativa</h4>
            <ul style={{ padding: 0, margin: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                <li key={i} style={{ display: 'flex', gap: '0.75rem', fontSize: '1.2rem', color: 'rgba(255,255,255,0.6)' }}>
          ...
          {history.length > 0 && (
             <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <h3 style={{ fontSize: '1.13rem', fontWeight: 700, color: '#fdf5e6', margin: 0 }}>MIS ÚLTIMOS TURNOS</h3>
              </div>
              <div style={{ padding: '0.5rem' }}>
                {history.map((r: any) => (
                  <div key={r.id} style={{ padding: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ margin: 0, fontSize: '1.05rem', color: 'rgba(255,255,255,0.4)' }}>{formatDate(r.hora_entrada)}</p>
                      <p style={{ margin: 0, fontSize: '1.2rem', color: '#fdf5e6' }}>{formatTime(r.hora_entrada)} - {r.hora_salida ? formatTime(r.hora_salida) : '...'}</p>
                    </div>
                    <span style={{ color: '#fbbf24', fontSize: '1.2rem', fontWeight: 700 }}>{r.total_horas}h</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              <div>
                <p style={{ margin: '0 0 0.2rem', fontSize: '0.83rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)' }}>Admin</p>
                <h3 style={{ color: '#fdf5e6', margin: 0, fontSize: '1.5rem', fontFamily: 'inherit' }}>Modificar Asistencia</h3>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '1.13rem', margin: '0.3rem 0 0' }}>
                  Registro de: <b style={{ color: '#fbbf24' }}>{editingRecord.nombre_completo}</b>
                </p>
              </div>
              <button
                onClick={() => setEditingRecord(null)}
                style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.4)', padding: '0.3rem 0.6rem', cursor: 'pointer', fontSize: '1.2rem', fontFamily: 'inherit', flexShrink: 0, marginLeft: '1rem' }}
            ...
            <form onSubmit={handleUpdateRecord} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Hora de Entrada</label>
                <input
                  type="datetime-local"
                  value={(() => {
                    try {
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Hora de Salida</label>
                <input
                  type="datetime-local"
                  value={(() => {
                    try {
              ...
              <div style={{ display: 'flex', gap: '0.75rem', paddingTop: '0.5rem' }}>
                <button
                  ...
                  style={{
                    ...
                    padding: '0.7rem', fontSize: '1.08rem', fontWeight: 700,
