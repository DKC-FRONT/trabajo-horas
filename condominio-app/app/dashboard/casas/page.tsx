...
export default function CasasPage() {
  ...
  const inputStyle = (field: string, hasError = false): React.CSSProperties => ({
    ...
    color: '#fdf5e6', fontSize: '1.23rem', padding: '0.65rem 0.85rem',
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        {!loading && (
          <div style={{ padding: '0.4rem 0.9rem', background: `${ACCENT}12`, border: `1px solid ${ACCENT}30`, fontSize: '0.98rem', color: ACCENT, letterSpacing: '0.1em', height: 'fit-content' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', marginBottom: '2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <p style={{ fontSize: '0.75rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255, 255, 255, 1)', margin: '0 0 0.2rem' }}>Registro</p>
          <h2 style={{ fontSize: '1.35rem', fontWeight: 700, color: '#fdf5e6', margin: 0 }}>AGREGAR CASA</h2>
        </div>
        <div style={{ padding: '1.25rem 1.5rem', display: 'flex', gap: '0.75rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '200px' }}>
            <label style={{ display: 'block', fontSize: '0.87rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: focusedField === 'nueva' ? ACCENT : 'rgba(255,255,255,0.4)', marginBottom: '0.4rem', transition: 'color 0.2s' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <p style={{ fontSize: '0.75rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255, 255, 255, 1)', margin: '0 0 0.2rem' }}>Directorio</p>
            <h2 style={{ fontSize: '1.35rem', fontWeight: 700, color: '#fdf5e6', margin: 0 }}>CASAS REGISTRADAS</h2>
          </div>
          <span style={{ fontSize: '0.98rem', color: 'rgba(255, 255, 255, 1)' }}>{casas.length} registros</span>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '3rem', textAlign: 'center' }}>
            <div style={{ fontSize: '2.7rem', marginBottom: '0.5rem', opacity: 0.25 }}>⬡</div>
            <div style={{ color: 'rgba(255, 255, 255, 1)', fontSize: '1.2rem', letterSpacing: '0.05em' }}>Sin casas registradas todavía</div>
          </div>
        ) : (
          <div>
            {casas.map((casa, i) => {
                  ...
                  {isEditing ? (
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1 }}>
                        <span style={{ fontSize: '0.87rem', color: 'rgba(255, 255, 255, 1)', letterSpacing: '0.1em', textTransform: 'uppercase', width: '30px' }}>Casa</span>
                        <input
                          ...
                          style={{ ...inputStyle('edit'), width: '120px', padding: '0.45rem 0.7rem', fontSize: '1.27rem' }}
                        />
                      </div>
                      <div style={{ display: 'flex', gap: '0.4rem' }}>
                        <button onClick={() => handleEditar(casa.id)} disabled={saving}
                          style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.3)', color: '#4ade80', padding: '0.4rem 0.85rem', fontSize: '0.98rem', letterSpacing: '0.1em', cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'inherit' }}
                          onMouseEnter={e => e.currentTarget.style.background = 'rgba(74,222,128,0.18)'}
                          onMouseLeave={e => e.currentTarget.style.background = 'rgba(74,222,128,0.08)'}>
                          ✓ Guardar
                        </button>
                        <button onClick={cancelEdit}
                          style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255, 255, 255, 1)', padding: '0.4rem 0.85rem', fontSize: '0.98rem', letterSpacing: '0.1em', cursor: 'pointer', fontFamily: 'inherit' }}
                    ...
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{
                          ...
                          fontSize: '1.35rem', color: ACCENT, flexShrink: 0,
                          transition: 'background 0.2s, border-color 0.2s',
                        }}>
                          ⬡
                        </div>
                        <div>
                          <div style={{ fontSize: '0.83rem', color: 'rgba(255, 255, 255, 1)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.1rem' }}>Unidad</div>
                          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fdf5e6', letterSpacing: '0.03em' }}>
                            {casa.numero_casa}
                          </div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: '0.4rem', opacity: isHov ? 1 : 0.6, transition: 'opacity 0.2s' }}>
                        <button onClick={() => startEdit(casa)}
                          style={{ background: 'transparent', border: `1px solid rgba(255,255,255,0.1)`, color: 'rgba(255, 255, 255, 1)', padding: '0.4rem 0.85rem', fontSize: '0.93rem', letterSpacing: '0.08em', cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit' }}
                        ...
                        <button onClick={() => handleEliminar(casa.id)} disabled={deletingId === casa.id}
                          style={{ background: 'transparent', border: '1px solid rgba(248,113,113,0.25)', color: 'rgba(248,113,113,0.7)', padding: '0.4rem 0.85rem', fontSize: '0.93rem', letterSpacing: '0.08em', cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit' }}
